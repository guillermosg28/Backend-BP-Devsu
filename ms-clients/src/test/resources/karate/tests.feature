Feature: Pruebas de la API DEVSU
  ...
  Scenario: Obtener información de la cuenta
    Given url API_URL + '/cuentas/1'
    When method get
    Then status 200

  Scenario: Actualizar estado de la cuenta
    Given url API_URL + '/cuentas/1'
    And request { estado: false }
    When method patch
    Then status 200
  ...
  Scenario: Crear una nueva transacción
    ...
    And request { cuentaId: 1, fecha: '2023-11-09', valor: -50 }
    When method post
    Then status 200
